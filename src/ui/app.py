...
This app takes a startup summary and returns:
    1. Three similar startup examples (from vector-store retrieval), each truncated to 200 words.
    2. A ~100-word market context generated by LLaMA-3.
    3. Four VC-style recommendations generated by LLaMA-3.

Changelog
---------
2025-06-02  • Add `postprocess_recommendations()` to hide prompt scaffolding lines
             and trim trailing hashtag blocks after the Team recommendation.
"""
...
def postprocess_recommendations(raw: str) -> str:
    """
    Clean the raw LLM output so the UI shows *only* the four bullet points.

    Steps
    -----
    1. Drop any lines that start with ``System:`` or ``Human:`` (prompt scaffolding).
    2. For the line starting with ``Team:``, remove everything from the first ``#`` onwards
       to eliminate trailing hashtag strings.
    ...
    -------
    str
        Sanitised, display-ready recommendations.
    """
    # -- 1. Remove prompt scaffolding -------------------------------------------------
    lines: List[str] = [
        ln for ln in raw.splitlines()
        if not re.match(r"^\s*(System|Human):", ln, flags=re.IGNORECASE)
    ]
    cleaned: str = "\n".join(lines)

    # -- 2. Truncate trailing hashtags on the Team recommendation ---------------------
    cleaned = re.sub(
        r"(Team:\s*.+?)(?:\s*#.*)$",         # greedy up to first '#'
        r"\1",
        cleaned,
...
if st.button("Evaluate Startup"):
    if not summary.strip():
        ...
        try:
            ...
            # ────────────────────────────────────────────────────────────────────
            st.subheader("📊 Market Context (from Internet)")
            ...
            if raw_recs:
                st.write(postprocess_recommendations(raw_recs))
            else:
