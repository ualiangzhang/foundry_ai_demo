...
def get_eval_chain() -> Any:
    ...
    Returns:
        Callable that expects {"question": summary}.
    """
...
###############################################################################
# Streamlit UI – Single “Evaluator” View                                       #
###############################################################################
...
# When the user clicks "Evaluate Startup"
if st.button("Evaluate Startup"):
    if not summary.strip():
        st.error("❗ Please enter a non-empty startup summary.")
    else:
        try:
            # Invoke the eval chain, which returns a dict containing:
