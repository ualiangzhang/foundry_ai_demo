...
if st.button("Evaluate Startup"):
    if not summary.strip():
        ...
        try:
            ...
            raw_recommendations: str = output.get("result", "")

            if raw_recommendations:
                # 1) Remove any leading prompt/instruction block up to "Market:"
                rec = raw_recommendations
                idx_market = rec.find("Market:")
                if idx_market != -1:
                    rec = rec[idx_market :]

                # 2) Truncate anything starting at the first "#" (hashtags)
                hash_idx = rec.find(" #")
                if hash_idx != -1:
                    rec = rec[:hash_idx].rstrip()
